...
### a)Create a scatterplot of mortality rate vs. level of nitric oxides. Do you think linear regression will fit these data well? Fit the regression and evaluate a residual plot from the regression.

```{r, echo=FALSE, fig.width=12, fig.height=6}
plot1 <- ggplot(pollution, aes(x = nox, y = mort)) + geom_point()
...
### b)Find an appropriate transformation that will result in data more appropriate for linear regression. Fit a regression to the transformed data and evaluate a new residual plot.
...
coefficients(model2)
```
For every increase of 1 in NOx, there is a multiplicative increase of e to the 15 in mortality. (This seems HUGE...)

### d) Now fit a model predicting mortality rate using levels of nitric oxides, sulfur dioxide, and hydrocarbons as inputs. Use appropriate transformations when helpful. Plot the fitted regression model and interpret the coeffiecients. 

```{r, echo=FALSE, fig.width=12, fig.height=6}
pollution2 <- mutate(pollution2, log_so2 = log(so2))
pollution2 <- mutate(pollution2, log_hc = log(hc))

model3 <- lm(mort ~ log_nox + log_so2 + log_hc, data = pollution2)
model3

ggplot(pollution2, aes(x = log_nox, y = mort)) + geom_point()
ggplot(pollution2, aes(x = log_so2, y = mort)) + geom_point()
ggplot(pollution2, aes(x = log_hc, y = mort)) + geom_point()

```
Is there a way to combine these plots somehow and show the regression line?
Also again the coefficients seem really big to me. Am I not interpretting them right? Each increase of 1 along the x axis corresponds to a multiplicative increase of e^coeff ?
...
essays <- select(profiles, is_female, contains("essay"))
...
orientations <- mutate(orientations, is_gay = ifelse(orientation == "gay", 1, 0))
orientations <- mutate(orientations, is_straight = ifelse(orientation == "straight", 1, 0))
...
bodytype <- mutate(bodytype, is_skinny = ifelse(body_type == "skinny", 1, 0))
bodytype <- mutate(bodytype, is_athletic = ifelse(body_type == "athletic", 1, 0))
bodytype <- mutate(bodytype, is_thin = ifelse(body_type == "thin", 1, 0))
bodytype <- mutate(bodytype, is_average = ifelse(body_type == "average", 1, 0))
bodytype <- mutate(bodytype, is_fit = ifelse(body_type == "fit", 1, 0))
bodytype <- mutate(bodytype, is_extra = ifelse(body_type == "a little extra", 1, 0))
bodytype <- mutate(bodytype, is_full = ifelse(body_type == "full figured", 1, 0))
bodytype <- mutate(bodytype, is_jacked = ifelse(body_type == "jacked", 1, 0))
bodytype <- mutate(bodytype, is_nope = ifelse(body_type == "rather not say", 1, 0))
...
profiles2 <- mutate(profiles2, drugs_sometimes = ifelse(drugs == "sometimes", 1, 0))
profiles2 <- mutate(profiles2, drugs_often = ifelse(drugs == "often", 1, 0))
...
drinking <- mutate(drinking, drinks_often = ifelse(drinks == "often", 1, 0))
drinking <- mutate(drinking, drinks_rarely = ifelse(drinks == "rarely", 1, 0))
drinking <- mutate(drinking, drinks_voften = ifelse(drinks == "very often", 1, 0))
drinking <- mutate(drinking, drinks_naa = ifelse(drinks == "not at all", 1, 0))
...
ggplot(prop_drinking, aes(x = drinking, y = prop, fill = sex)) + 
  geom_bar(stat = "identity", position = "dodge") +
...
read <- group_by(essays, is_female) %>% 
  summarise(prop_read=mean(has_read))
knitr::kable(read, col.names = c("Is Female", "Proportion of Respondents Whose Essay Contains 'Read'"))


#essay contains "cook"
essays$has_cook <-profile.has.query(data.frame = essays, query = "cook")

cook <- group_by(essays, is_female) %>% 
  summarise(prop_read=mean(has_cook))
knitr::kable(cook, col.names = c("Is Female", "Proportion of Respondents Whose Essay Contains 'Cook'"))
...
vehicles <- group_by(essays, is_female) %>% 
  summarise(prop_vehicles=mean(vehicles))
knitr::kable(vehicles, col.names = c("Is Female", "Proportion of Respondents Whose Essay Contains 'Cars', 'Truck', or 'Motorcycle'"))
...
kidwords <- group_by(essays, is_female) %>% 
  summarise(prop_kidwords = mean(kidwords))
knitr::kable(kidwords, col.names = c("Is Female", "Proportion of Respondents Whose Essay Contains 'Child' or 'Kid'"))
